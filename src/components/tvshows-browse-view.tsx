...
import { useDiscoverTV, usePopularTV, useTopRatedTV, useAiringTodayTV, useOnTheAirTV, useTVGenres } from "@/hooks/use-tmdb";
...
export function TVShowsBrowseView() {
  const { tvshowsBrowseOptions } = useNavigationStore();
  const [selectedGenre, setSelectedGenre] = useState<number | null>(tvshowsBrowseOptions.initialGenreId ?? null);
  const [sortBy, setSortBy] = useState("popularity.desc");
  const [activeTab, setActiveTab] = useState<TabKey>(
    (tvshowsBrowseOptions.initialTab as TabKey) ?? "popular"
  );
  ...
  const discoverQuery = useDiscoverTV({
    genreId: selectedGenre,
    sortBy,
    page,
  });

  const popularQuery = usePopularTV(page);
  const topRatedQuery = useTopRatedTV(page);
  ...
  const shows = activeQuery.data?.results ?? [];
    ...
    >
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-[1440px] mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-exbed text-3xl sm:text-4xl tracking-wider">TV Shows</h1>
            <p className="text-sm text-muted-foreground mt-1">
              {totalResults > 0 ? `${totalResults.toLocaleString()} shows` : "Browse TV shows"}
            </p>
          </div>
          {/* Sort dropdown */}
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-[1440px] mx-auto mt-6">
        {activeQuery.isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-lg font-medium text-muted-foreground">No TV shows found</p>
